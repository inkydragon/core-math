...
def number_failures_x(S1,C1,S2,C2,x):
   ...
   for r in 'NZUD':
      ...
      y = cos(R11(x))
      if is_subnormal(y):
         y = cos_subnormal(x,y,r)
      t = fma(R24(C1[i1]),R24(C2[i2]),R24(S1[i1])*R24(S2[i2]))
      z = R11(t)
      if is_subnormal(z):
         z = subnormalize(t,r)
      # print (r, get_hex(y), get_hex(z))
      if y!=z:
         n += 1
   return n
...
def optimize(verbose=true):
   l = ["0x1.0a4p+9","-0x1.0a4p+9","0x1.dfp-5","-0x1.dfp-5","0x1.3cp+14","-0x1.3cp+14","0x0p+0","-0x0p+0","0x1.63p+8","-0x1.63p+8","0x1.63p+10","-0x1.63p+10","0x1.70cp+10","-0x1.70cp+10","0x1.124p+8","-0x1.124p+8","0x1.63p+8","-0x1.63p+8","0x1.63p+9","-0x1.63p+9","0x1.63p+10","-0x1.63p+10","0x1.70cp+10","-0x1.70cp+10","0x1.63p+11","-0x1.63p+11","0x1.124p+8","-0x1.124p+8","0x1.b3p+15","-0x1.b3p+15","0x0p+0","-0x0p+0","0x1.63p+8","-0x1.63p+8","0x1.39p+10","-0x1.39p+10","0x1.63p+10","-0x1.63p+10","0x1.bbcp+10","-0x1.bbcp+10"]
   l2 = []
   for x in l:
      if not x in l2:
         l2.append(x)
